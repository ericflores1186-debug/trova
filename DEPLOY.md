...
TikTok and Instagram use the same proxy settings, but only as a fallback: each
request goes direct first and switches to the proxy if it is refused, so the
metered proxy is not spent on requests that did not need it. If TikTok starts
refusing both, Render's logs say which route failed and whether TikTok served a
bot check. Instagram is the more likely of the two to start refusing server
traffic; the logs name the route and whether Instagram answered at all.
